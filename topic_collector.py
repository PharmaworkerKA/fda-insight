"""FDA規制インサイト - トピック自動収集モジュール

NEWS_SOURCESからRSS + スクレイピングで最新記事を収集し、
Gemini APIでFDA規制関連のネタだけフィルタリング＆ランク付けする。
"""
import json
import logging
import re
...
class TopicCollector:
    ...
    def _filter_and_rank(self, items: list[dict]) -> list[dict]:
        """Gemini APIでFDA規制関連のネタをフィルタリング＆ランク付けする"""
...
【評価基準】
- FDA承認・規制・ガイダンスに直接関連する記事: スコア80-100
- 薬事申請・GMP・データインテグリティに関連する記事: スコア60-79
- 間接的に医薬品規制に関連する記事: スコア40-59
- 関連性が低い記事: 除外
...
```json
[
  {{"index": 1, "score": 95, "category": "FDA最新ニュース", "blog_title_suggestion": "ブログ記事タイトル案", "keyword": "SEOキーワード"}}
]
```
スコア40未満の記事は含めないでください。"""
        ...
        try:
            ...
            for r in ranked:
                ...
                if 0 <= idx < len(items):
                    ...
                    item["category"] = r.get("category", "FDA最新ニュース")
    ...
    def _simple_filter(self, items: list[dict]) -> list[dict]:
        """簡易キーワードベースのフィルタリング"""
        keywords = [
            "fda", "nda", "bla", "510k", "warning letter",
            "guidance", "cder", "cber", "pmda", "ectd", "regulatory",
        ]
        result = []
        for item in items:
